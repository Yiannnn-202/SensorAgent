...
void Arm_Close(void) { Rm_Api.rm_delete_robot_arm(robot_handle); }
...
void RmArm::Arm_MoveJ_Callback(rm_ros_interfaces::msg::Movej::SharedPtr msg) {
  ...
  if (msg->dof != 6 && msg->dof != 7) {
    ...
    RCLCPP_ERROR(this->get_logger(), "MoveJ rejected: unsupported dof=%u, only 6 or 7 are valid.",
                 msg->dof);
    return;
  }
  ...
  if (joint_size < expected_joint_size) {
    ...
    RCLCPP_ERROR(this->get_logger(),
                 ...
                 msg->dof, expected_joint_size, joint_size);
    return;
  }
  ...
  if (msg->dof != arm_dof_g) {
    ...
    this->MoveJ_Cmd_Result->publish(movej_result);
    RCLCPP_ERROR(this->get_logger(),
                 "MoveJ rejected: message dof=%u does not match configured arm_dof=%d.", msg->dof,
                 arm_dof_g);
    return;
  }
...
void RmArm::Arm_MoveC_Callback(rm_ros_interfaces::msg::Movec::SharedPtr msg) {
  ...
  block = msg->block;
  trajectory_connect = msg->trajectory_connect;
  // res = Rm_Api.Service_Movec_Cmd(m_sockhand, pose_via, pose_to, v, 0, loop, trajectory_connect,
  // block);
...
void RmArm::Arm_Movej_CANFD_Callback(rm_ros_interfaces::msg::Jointpos::SharedPtr msg) {
  ...
  //  res = Rm_Api.Service_Movej_CANFD_With_Radio(m_sockhand, joint, follow, expand,
  //  trajectory_mode, radio); if(stop_flag == false)
...
void RmArm::Arm_Movej_CANFD_Custom_Callback(rm_ros_interfaces::msg::Jointposcustom::SharedPtr msg) {
  ...
  // res = Rm_Api.Service_Movej_CANFD_With_Radio(m_sockhand, joint, follow, expand, trajectory_mode,
  // radio);
  res = Rm_Api.rm_movej_canfd(robot_handle, joint, follow, expand, trajectory_mode, radio);
...
void RmArm::Set_UDP_Configuration(int udp_cycle, int udp_port, int udp_force_coordinate,
  ...
  // RCLCPP_INFO (this->get_logger(),"custom_config is %d speed is
  // %d\n",config.custom_config.joint_speed,udp_joint_speed_state_);
  ...
  if (res == 0) {
    RCLCPP_INFO(this->get_logger(),
                "UDP_Configuration is "
                "cycle:%dms,port:%d,force_coordinate:%d,ip:%s,hand:%d,rm_plus_base:%d,rm_plus_"
                "state:%d,speed:%d\n",
                udp_cycle, udp_port, udp_force_coordinate, udp_ip.c_str(), udp_hand_g,
                rm_plus_base_g, rm_plus_state_g, config.custom_config.joint_speed);
  } else {
    RCLCPP_INFO(this->get_logger(), "The set_realtime_push error code is %d\n", res);
  }
}
...
//     copy = msg;
//     res = Rm_Api.Service_Get_Arm_Software_Version(m_sockhand, plan_version, ctrl_version,
//     kernal1, kernal2, product_version); if(res == 0)
//     {
...
void RmArm::Arm_Get_Arm_Software_Info_Callback(const std_msgs::msg::Empty::SharedPtr msg) {
  ...
  copy = msg;
  res = Rm_Api.rm_get_arm_software_info(robot_handle, &arm_software_info);
  // res = Rm_Api.Service_Get_Arm_Software_Version(m_sockhand, plan_version, ctrl_version, kernal1,
  // kernal2, product_version);
  if (res == 0) {
...
void RmArm::Arm_Get_Joint_Software_Version_Callback(const std_msgs::msg::Empty::SharedPtr msg) {
  ...
  if (res == 0) {
    if (controller_version == 3) {
      for (int i = 0; i < 6; i++) {
        jointsoftwareinfo.joint_version[i] =
            std::to_string(joint_software_info[i]);  //这里需要先转换成十六进制再转换成字符串
      }
      if (arm_dof_g == 7) {
    ...
    const rm_ros_interfaces::msg::Gettrajectorylist::SharedPtr msg)  //查询轨迹列表
{
  ...
  res = Rm_Api.rm_get_trajectory_file_list(robot_handle, page_num, page_size, vague_search,
...
void RmArm::Set_Run_Trajectory_Callback(
    const std_msgs::msg::String::SharedPtr msg)  //开始运行指定轨迹
{
  int32_t res;
...
void RmArm::Delete_Trajectory_File_Callback(
    const std_msgs::msg::String::SharedPtr msg)  //删除指定轨迹
{
  int32_t res;
...
void RmArm::Save_Trajectory_File_Callback(
    const std_msgs::msg::String::SharedPtr msg)  //保存轨迹到控制机器
{
  int32_t res;
...
void RmArm::Get_Modbus_Tcp_Master_Callback(
    const rm_ros_interfaces::msg::Mastername::SharedPtr msg) {
  int32_t res;
  rm_modbus_tcp_master_info_t master;
...
void RmArm::Get_Modbus_Tcp_Master_List_Callback(
  ...
  res = Rm_Api.rm_get_modbus_tcp_master_list(robot_handle, page_num, page_size, vague_search,
                                             &master_list);
  ...
  if (controller_type == 4) {
    if ((param.num == int(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(this->get_logger(),
                  "The write_modbus_rtu_coils num is diff with data size use data size %d\n",
                  param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
...
void RmArm::Read_Modbus_RTU_Input_Status_Callback(
  ...
  if (controller_type == 4) {
    ...
    if (params_coils.num <= 8 && params_coils.num > 0) {
      ...
      // RCLCPP_INFO (this->get_logger(),"data is %d,controller_type is
...
void RmArm::Read_Modbus_RTU_Holding_Registers_Callback(
  ...
  if (controller_type == 4) {
    if (param.num > 100) {
      param.num = 100;
      RCLCPP_WARN(this->get_logger(),
                  "The read_modbus_rtu_holding_registers num is over 100 use 100\n");
    }
    ...
    if (param.num <= 1) {
      ...
      if (param.num > 12) {
        param.num = 12;
        RCLCPP_WARN(this->get_logger(),
                    "The read_modbus_rtu_holding_registers num is over 12 use 12\n");
      }
      params_coils.num = param.num;
...
void RmArm::Write_Modbus_RTU_Registers_Callback(
  ...
  if (controller_type == 4) {
    if ((param.num == int(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(this->get_logger(),
                  "The write_modbus_rtu_registers num is diff with data size use data size %d\n",
                  param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
...
void RmArm::Read_Modbus_RTU_Input_Registers_Callback(
  ...
  if (controller_type == 4) {
    if (param.num > 100) {
      param.num = 100;
      RCLCPP_WARN(this->get_logger(),
                  "The read_modbus_rtu_input_registers num is over 100 use 100\n");
    }
    ...
    if (param.num <= 1) {
      ...
      if (param.num > 12) {
        param.num = 12;
        RCLCPP_WARN(this->get_logger(),
                    "The read_modbus_rtu_input_registers num is over 12 use 12\n");
      }
      params_coils.num = param.num;
...
void RmArm::Write_Modbus_TCP_Coils_Callback(
  ...
  if (controller_type == 4) {
    if ((param.num == int(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(this->get_logger(),
                  "The write_modbus_tcp_coils num is diff with data size use data size %d\n",
                  param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
...
void RmArm::Read_Modbus_TCP_Holding_Registers_Callback(
  ...
  if (controller_type == 4) {
    if (param.num > 100) {
      param.num = 100;
      RCLCPP_WARN(this->get_logger(),
                  "The read_modbus_tcp_holding_registers num is over 100 use 100\n");
    }
...
void RmArm::Write_Modbus_TCP_Registers_Callback(
  ...
  if (controller_type == 4) {
    if ((param.num == int(msg->data.size())) && (param.num <= 100)) {
      ...
      RCLCPP_WARN(this->get_logger(),
                  "The write_modbus_tcp_registers num is diff with data size use data size %d\n",
                  param.num);
    } else if (msg->data.size() > 100) {
      param.num = 100;
      for (int i = 0; i < 100; i++) {
...
void RmArm::Read_Modbus_TCP_Input_Registers_Callback(
  ...
  if (controller_type == 4) {
    if (param.num > 100) {
      param.num = 100;
      RCLCPP_WARN(this->get_logger(),
                  "The read_modbus_tcp_input_registers num is over 100 use 100\n");
    }
...
// ------------------------------------------Modbus相关
// end------------------------------------------
void RmArm::Send_Project_Callback(
    const rm_ros_interfaces::msg::Sendproject::SharedPtr msg)  //文件下发
{
  int32_t res;
  ...
  if (res == 0) {
    ...
    if (res == 1) {
      if (errline == 0)
        RCLCPP_INFO(this->get_logger(),
                    "The rm_send_project length of the verification data is incorrect");
      else if (errline == -1) {
        RCLCPP_INFO(this->get_logger(), "rm_send_project no error");
      } else {
        RCLCPP_INFO(this->get_logger(),
                    "The rm_send_project number of problematic engineering lines：%d", errline);
      }
    }
  }
}
void RmArm::Get_Program_Run_State_Callback(
    const std_msgs::msg::Empty::SharedPtr msg)  //查询在线编程运行状态
{
  int32_t res;
  copy = msg;
...
void RmArm::Arm_Force_Position_Move_Joint_Callback(
  ...
  force = msg->force;
  follow = msg->follow;
  // res = Rm_Api.Service_Force_Position_Move_Joint(m_sockhand, joint, sensor, mode, dir, force,
  // follow);
...
// void RmArm::Arm_Force_Position_Move_Joint_75_Callback(const
// rm_ros_interfaces::msg::Forcepositionmovejoint75::SharedPtr msg)
// {
//     int32_t res;
//     float joint[7];
//     byte sensor;
...
void RmArm::Arm_Force_Position_Move_Pose_Callback(
  ...
  force = msg->force;
  follow = msg->follow;
  // res = Rm_Api.Service_Force_Position_Move_Pose(m_sockhand, joint_pose, sensor, mode, dir, force,
  // follow);
  res = Rm_Api.rm_force_position_move_pose(robot_handle, joint_pose, sensor, mode, dir, force,
                                           follow);
  if (res != 0) {
...
void RmArm::Arm_Set_Gripper_Pick_Callback(
    const rm_ros_interfaces::msg::Gripperpick::SharedPtr msg) {
  int speed;
  int force;
  bool block;
...
void RmArm::Arm_Set_Hand_Posture_Callback(
    const rm_ros_interfaces::msg::Handposture::SharedPtr msg) {
  int posture_num;
  bool block;
  int timeout;
  int32_t res;
...
void RmArm::Arm_Set_Hand_Follow_Pos_Callback(
    const rm_ros_interfaces::msg::Handangle::SharedPtr msg) {
  int pos[6];
  int block;
  int32_t res;
...
void RmArm::Arm_Get_Force_Data_Callback(const std_msgs::msg::Empty::SharedPtr msg) {
  ...
  copy = msg;
  int32_t res;
  // res = Rm_Api.Service_Get_Force_Data(m_sockhand, force_data, zero_force_data, work_zero_data,
  // tool_zero_data);
...
void Udp_Robot_Status_Callback(rm_realtime_arm_joint_state_t data) {
  ...
  // RCLCPP_INFO (this->get_logger(),"speed %f %f %f %f %f
  ...
  // std::cout<<"joint_speed is
  // "<<data.joint_status.joint_speed[0]<<data.joint_status.joint_speed[1]<<data.joint_status.joint_speed[2]<<data.joint_status.joint_speed[3]<<data.joint_status.joint_speed[4]<<data.joint_status.joint_speed[5]<<std::endl;
  if (arm_dof_g == 7) {
...
UdpPublisherNode::UdpPublisherNode() : rclcpp::Node("udp_publish_node") {
  ...
  Heart_Timer = this->create_wall_timer(std::chrono::milliseconds(500),
                                        std::bind(&UdpPublisherNode::heart_timer_callback, this),
                                        callback_group_time2_);
  /********************************************************************UDP传输数据**********************************************************/
  Joint_Position_Result = this->create_publisher<sensor_msgs::msg::JointState>(
      "joint_states", 10);  //发布当前的关节角度
  ...
  // Sys_Err_Result = this->create_publisher<std_msgs::msg::UInt16>("rm_driver/udp_sys_err", 10);
  // //发布当前的系统错误码
  Rm_Err_Result = this->create_publisher<rm_ros_interfaces::msg::Rmerr>(
      "rm_driver/udp_rm_err", 10);  //发布当前的机械臂错误码
  ...
  Joint_Speed_Result =
      this->create_publisher<rm_ros_interfaces::msg::Jointspeed>("rm_driver/udp_joint_speed", 10);
  ...
  Rm_Plus_Base_Result =
      this->create_publisher<rm_ros_interfaces::msg::Rmplusbase>("rm_driver/udp_rm_plus_base", 10);
  Rm_Plus_State_Result = this->create_publisher<rm_ros_interfaces::msg::Rmplusstate>(
      "rm_driver/udp_rm_plus_state", 10);
  Lift_State_Result =
      this->create_publisher<rm_ros_interfaces::msg::Udpliftstate>("rm_driver/udp_lift_state", 10);
  Expand_State_Result = this->create_publisher<rm_ros_interfaces::msg::Udpexpandstate>(
      "rm_driver/udp_expand_state", 10);
  Aloha_State_Result =
      this->create_publisher<rm_ros_interfaces::msg::Alohastate>("rm_driver/udp_aloha_state", 10);
}
...
RmArm::RmArm() : rclcpp::Node("rm_driver") {
  ...
  Get_Realtime_Push_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Realtime_Push_Callback, this, std::placeholders::_1), sub_opt2);
  /******************************************************设置udp配置********************************************************************/
  ...
  Set_Realtime_Push_Cmd = this->create_subscription<rm_ros_interfaces::msg::Setrealtimepush>(
      ...
      std::bind(&RmArm::Arm_Set_Realtime_Push_Callback, this, std::placeholders::_1), sub_opt2);
  /******************************************************************************end*******************************************************************/

  /***********************************************************************运动配置**********************************************************************/
  /****************************************MoveJ运动控制*************************************/
  MoveJ_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/movej_result",
  ...
  /****************************************MoveL运动控制*************************************/
  MoveL_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/movel_result",
                                                                 rclcpp::ParametersQoS());
  ...
  MoveL_offset_Cmd = this->create_subscription<rm_ros_interfaces::msg::Moveloffset>(
      ...
      std::bind(&RmArm::Arm_MoveL_Offset_Callback, this, std::placeholders::_1), sub_opt4);
  /****************************************MoveC运动控制*************************************/
  MoveC_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/movec_result",
                                                                 rclcpp::ParametersQoS());
  MoveC_Cmd = this->create_subscription<rm_ros_interfaces::msg::Movec>(
      "rm_driver/movec_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Arm_MoveC_Callback, this, std::placeholders::_1), sub_opt4);
  /******************************************角度透传*****************************************/
  ...
  Movej_CANFD_Custom_Cmd = this->create_subscription<rm_ros_interfaces::msg::Jointposcustom>(
      ...
      std::bind(&RmArm::Arm_Movej_CANFD_Custom_Callback, this, std::placeholders::_1), sub_opt4);
  /*******************************************位姿透传****************************************/
  Movep_CANFD_Cmd = this->create_subscription<rm_ros_interfaces::msg::Cartepos>(
      "rm_driver/movep_canfd_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Arm_Movep_CANFD_Callback, this, std::placeholders::_1), sub_opt4);
  Movep_CANFD_Custom_Cmd = this->create_subscription<rm_ros_interfaces::msg::Carteposcustom>(
      ...
      std::bind(&RmArm::Arm_Movep_CANFD_Custom_Callback, this, std::placeholders::_1), sub_opt4);
  /****************************************MoveJ_P运动控制*************************************/
  MoveJ_P_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/movej_p_result",
                                                                   rclcpp::ParametersQoS());
  MoveJ_P_Cmd = this->create_subscription<rm_ros_interfaces::msg::Movejp>(
      "rm_driver/movej_p_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Arm_MoveJ_P_Callback, this, std::placeholders::_1), sub_opt4);
  /***********************************************轨迹急停****************************************/
  Move_Stop_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/move_stop_result",
                                                                     rclcpp::ParametersQoS());
  Move_Stop_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      "rm_driver/move_stop_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Arm_Move_Stop_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Arm_Emergency_Stop_Cmd = this->create_subscription<rm_ros_interfaces::msg::Stop>(
      ...
      std::bind(&RmArm::Arm_Emergency_Stop_Callback, this, std::placeholders::_1), sub_opt2);
  /***********************************************轨迹暂停****************************************/
  Arm_Pause_Cmd_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/pause_result",
                                                                     rclcpp::ParametersQoS());
  Arm_Pause_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      "rm_driver/pause_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Pause_Callback, this, std::placeholders::_1), sub_opt2);
  /***********************************************轨迹暂停后继续轨迹运动****************************************/
  ...
  Set_Arm_Continue_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Set_Arm_Continue_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Set_Joint_Teach_Cmd = this->create_subscription<rm_ros_interfaces::msg::Jointteach>(
      ...
      std::bind(&RmArm::Set_Joint_Teach_Callback, this, std::placeholders::_1), sub_opt4);
  /*********************************************************位置示教*****************************************************************/
  ...
  Set_Pos_Teach_Cmd = this->create_subscription<rm_ros_interfaces::msg::Posteach>(
      ...
      std::bind(&RmArm::Set_Pos_Teach_Callback, this, std::placeholders::_1), sub_opt4);
  /*********************************************************姿态示教*****************************************************************/
  ...
  Set_Ort_Teach_Cmd = this->create_subscription<rm_ros_interfaces::msg::Ortteach>(
      ...
      std::bind(&RmArm::Set_Ort_Teach_Callback, this, std::placeholders::_1), sub_opt4);
  /*********************************************************示教停止*****************************************************************/
  ...
  Set_Stop_Teach_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Set_Stop_Teach_Callback, this, std::placeholders::_1), sub_opt2);
  /******************************************************************************end*****************************************************************/

  /******************************************************************************四代控制器新增*****************************************************************/
  /************************************************************************查询机械臂基本信息***************************************************************/
  // if(controller_version==4){
  //     Get_Arm_Software_Version_Result_v4 =
  //     this->create_publisher<rm_ros_interfaces::msg::Armsoftversionv4>("rm_driver/get_arm_software_version_result",
  ...
  //         sub_opt2);}
  // if(controller_version==3){
  //     Get_Arm_Software_Version_Result_v3 =
  //     this->create_publisher<rm_ros_interfaces::msg::Armsoftversionv3>("rm_driver/get_arm_software_version_result",
  //     rclcpp::ParametersQoS()); Get_Arm_Software_Version_Cmd =
  //     this->create_subscription<std_msgs::msg::Empty>("rm_driver/get_arm_software_version_cmd",rclcpp::ParametersQoS(),
  //         std::bind(&RmArm::Arm_Get_Arm_Software_Info_Callback,this,std::placeholders::_1),
  //         sub_opt2);}
  ...
  Get_Arm_Software_Version_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Arm_Software_Info_Callback, this, std::placeholders::_1), sub_opt2);
  /************************************************************************查询软件版本***************************************************************/
  ...
  Get_Robot_Info_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Robot_Info_Callback, this, std::placeholders::_1), sub_opt2);
  /************************************************************************查询关节软件版本***************************************************************/
  ...
  Set_Run_Trajectory_Cmd = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&RmArm::Set_Run_Trajectory_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Delete_Trajectory_File_Cmd = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&RmArm::Delete_Trajectory_File_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Save_Trajectory_File_Cmd = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&RmArm::Save_Trajectory_File_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Set_Controller_RS485_Mode_Cmd = this->create_subscription<rm_ros_interfaces::msg::RS485params>(
      ...
      std::bind(&RmArm::Set_Controller_RS485_Mode_Callback, this, std::placeholders::_1), sub_opt5);
  // Add_Modbus_Tcp_Master_Result =
  // this->create_publisher<std_msgs::msg::Bool>("rm_driver/add_modbus_tcp_master_result",
  // rclcpp::ParametersQoS()); Add_Modbus_Tcp_Master_Cmd =
  // this->create_subscription<rm_ros_interfaces::msg::Modbustcpmasterinfo>("rm_driver/add_modbus_tcp_master_cmd",rclcpp::ParametersQoS(),
  //     std::bind(&RmArm::Add_Modbus_Tcp_Master_Callback,this,std::placeholders::_1),
  //     sub_opt5);
  // Delete_Modbus_Tcp_Master_Result =
  // this->create_publisher<std_msgs::msg::Bool>("rm_driver/delete_modbus_tcp_master_result",
  // rclcpp::ParametersQoS()); Delete_Modbus_Tcp_Master_Cmd =
  // this->create_subscription<rm_ros_interfaces::msg::Mastername>("rm_driver/delete_modbus_tcp_master_cmd",rclcpp::ParametersQoS(),
  ...
  if (controller_type == 4) {
    ...
    Get_Modbus_Tcp_Master_Cmd = this->create_subscription<rm_ros_interfaces::msg::Mastername>(
        ...
        std::bind(&RmArm::Get_Modbus_Tcp_Master_Callback, this, std::placeholders::_1), sub_opt5);
    /***********************************************************************查询modbus主站列表***********************************************************************/
    ...
    Get_Controller_RS485_Mode_v4_Result =
        this->create_publisher<rm_ros_interfaces::msg::RS485params>(
    ...
    Set_Tool_RS485_Mode_Cmd = this->create_subscription<rm_ros_interfaces::msg::RS485params>(
        ...
        std::bind(&RmArm::Set_Tool_RS485_Mode_Callback, this, std::placeholders::_1), sub_opt5);
    ...
    Get_Tool_RS485_Mode_v4_Cmd = this->create_subscription<std_msgs::msg::Empty>(
        ...
        std::bind(&RmArm::Get_Tool_RS485_Mode_v4_Callback, this, std::placeholders::_1), sub_opt2);
  } else {
    // Set_Controller_RS485_Mode_Cmd =
    // this->create_subscription<rm_ros_interfaces::msg::RS485params>("rm_driver/set_controller_rs485_mode_cmd",rclcpp::ParametersQoS(),
  ...
  Read_Modbus_RTU_Coils_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbusrtureadparams>(
          ...
          std::bind(&RmArm::Read_Modbus_RTU_Coils_Callback, this, std::placeholders::_1), sub_opt2);
  /***********************************************************************Modbus
   * RTU协议写线圈***********************************************************************/
  ...
  Read_Modbus_RTU_Holding_Registers_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbusrtureadparams>(
          ...
          std::bind(&RmArm::Read_Modbus_RTU_Holding_Registers_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /***********************************************************************Modbus
   * RTU协议写保持寄存器***********************************************************************/
  ...
  Read_Modbus_TCP_Coils_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbustcpreadparams>(
          ...
          std::bind(&RmArm::Read_Modbus_TCP_Coils_Callback, this, std::placeholders::_1), sub_opt2);
  /***********************************************************************Modbus
   * TCP协议写线圈***********************************************************************/
  ...
  Read_Modbus_TCP_Holding_Registers_Cmd =
      this->create_subscription<rm_ros_interfaces::msg::Modbustcpreadparams>(
          ...
          std::bind(&RmArm::Read_Modbus_TCP_Holding_Registers_Callback, this,
                    std::placeholders::_1),
          sub_opt2);
  /***********************************************************************Modbus
   * TCP协议写保持寄存器***********************************************************************/
  ...
  /***********************************************************************文件下发***********************************************************************/
  Send_Project_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/send_project_result",
                                                                    rclcpp::ParametersQoS());
  Send_Project_Cmd = this->create_subscription<rm_ros_interfaces::msg::Sendproject>(
      ...
      std::bind(&RmArm::Send_Project_Callback, this, std::placeholders::_1), sub_opt2);
  /***********************************************************************查询在线编程运行状态***********************************************************************/
  ...
  Get_Program_Run_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Get_Program_Run_State_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Force_Position_Move_Cmd = this->create_subscription<rm_ros_interfaces::msg::Forcepositionmove>(
      ...
      std::bind(&RmArm::Arm_Force_Position_Move_Callback, this, std::placeholders::_1), sub_opt4);
  /********************************************************设置力位混合控制*******************************************************************/
  ...
  Set_Force_Postion_Cmd = this->create_subscription<rm_ros_interfaces::msg::Setforceposition>(
      ...
      std::bind(&RmArm::Arm_Set_Force_Postion_Callback, this, std::placeholders::_1), sub_opt4);
  /********************************************************结束力位混合控制*******************************************************************/
  ...
  Stop_Force_Postion_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Stop_Force_Postion_Callback, this, std::placeholders::_1), sub_opt4);
  /****************************************************************************end******************************************************************/
  ...
  Change_Work_Frame_Cmd = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&RmArm::Arm_Change_Work_Frame_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Change_Tool_Frame_Cmd = this->create_subscription<std_msgs::msg::String>(
      ...
      std::bind(&RmArm::Arm_Change_Tool_Frame_Callback, this, std::placeholders::_1), sub_opt2);
  ...
  Get_Curr_WorkFrame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Curr_WorkFrame_Callback, this, std::placeholders::_1), sub_opt2);
  /**********************************************************获得工具坐标系********************************************************************/
  ...
  Get_All_Tool_Frame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_All_Tool_Frame_Callback, this, std::placeholders::_1), sub_opt2);
  /**********************************************************获得所有工作坐标系********************************************************************/
  ...
  Get_All_Work_Frame_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_All_Work_Frame_Callback, this, std::placeholders::_1), sub_opt2);
  /*****************************************************************************end***************************************************************/
  ...
  Set_Tool_Voltage_Cmd = this->create_subscription<std_msgs::msg::UInt16>(
      ...
      std::bind(&RmArm::Arm_Set_Tool_Voltage_Callback, this, std::placeholders::_1), sub_opt2);
  /*****************************************************************************end***************************************************************/
  ...
  Set_Joint_Err_Clear_Cmd = this->create_subscription<rm_ros_interfaces::msg::Jointerrclear>(
      ...
      std::bind(&RmArm::Arm_Set_Joint_Err_Clear_Callback, this, std::placeholders::_1), sub_opt2);
  /*****************************************************************************end***************************************************************/
  ...
  Set_Gripper_Pick_On_Cmd = this->create_subscription<rm_ros_interfaces::msg::Gripperpick>(
      ...
      std::bind(&RmArm::Arm_Set_Gripper_Pick_On_Callback, this, std::placeholders::_1), sub_opt3);
  /********************************************手爪力控夹取**********************************/
  ...
  Set_Gripper_Pick_Cmd = this->create_subscription<rm_ros_interfaces::msg::Gripperpick>(
      ...
      std::bind(&RmArm::Arm_Set_Gripper_Pick_Callback, this, std::placeholders::_1), sub_opt3);
  /*****************************************手爪到达指定位置**********************************/
  ...
  Set_Gripper_Position_Cmd = this->create_subscription<rm_ros_interfaces::msg::Gripperset>(
      ...
      std::bind(&RmArm::Arm_Set_Gripper_Position_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Set_Hand_Posture_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handposture>(
      ...
      std::bind(&RmArm::Arm_Set_Hand_Posture_Callback, this, std::placeholders::_1), sub_opt3);
  /***************************************设置灵巧手动作序列序号*********************************/
  Set_Hand_Seq_Result = this->create_publisher<std_msgs::msg::Bool>("rm_driver/set_hand_seq_result",
                                                                    rclcpp::ParametersQoS());
  Set_Hand_Seq_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handseq>(
      "rm_driver/set_hand_seq_cmd", rclcpp::ParametersQoS(),
      std::bind(&RmArm::Arm_Set_Hand_Seq_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************设置灵巧手角度************************************/
  ...
  Set_Hand_Angle_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handangle>(
      ...
      std::bind(&RmArm::Arm_Set_Hand_Angle_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************设置灵巧手速度************************************/
  ...
  Set_Hand_Speed_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handspeed>(
      ...
      std::bind(&RmArm::Arm_Set_Hand_Speed_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************设置灵巧手力度************************************/
  ...
  Set_Hand_Force_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handforce>(
      ...
      std::bind(&RmArm::Arm_Set_Hand_Force_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************设置灵巧手角度跟随************************************/
  ...
  Set_Hand_Follow_Angle_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handangle>(
      ...
      std::bind(&RmArm::Arm_Set_Hand_Follow_Angle_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************设置灵巧手姿势跟随************************************/
  ...
  Set_Hand_Follow_Pos_Cmd = this->create_subscription<rm_ros_interfaces::msg::Handangle>(
      ...
      std::bind(&RmArm::Arm_Set_Hand_Follow_Pos_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Set_Lift_Speed_Cmd = this->create_subscription<rm_ros_interfaces::msg::Liftspeed>(
      ...
      std::bind(&RmArm::Arm_Set_Lift_Speed_Callback, this, std::placeholders::_1), sub_opt3);
  /****************************************设置升降机构高度**********************************/
  ...
  Set_Lift_Height_Cmd = this->create_subscription<rm_ros_interfaces::msg::Liftheight>(
      ...
      std::bind(&RmArm::Arm_Set_Lift_Height_Callback, this, std::placeholders::_1), sub_opt3);
  /****************************************获取升降机构状态**********************************/
  ...
  Get_Lift_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Lift_State_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Set_Expand_Speed_Cmd = this->create_subscription<std_msgs::msg::Int32>(
      ...
      std::bind(&RmArm::Arm_Set_Expand_Speed_Callback, this, std::placeholders::_1), sub_opt3);
  /****************************************设置拓展关节高度**********************************/
  ...
  Set_Expand_Pos_Cmd = this->create_subscription<rm_ros_interfaces::msg::Expandpos>(
      ...
      std::bind(&RmArm::Arm_Set_Expand_Pos_Callback, this, std::placeholders::_1), sub_opt3);
  /****************************************获取拓展关节状态**********************************/
  ...
  Get_Expand_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Expand_State_Callback, this, std::placeholders::_1), sub_opt3);
  /*******************************************************************************end*****************************************************************/
  ...
  Get_Current_Arm_State_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Current_Arm_State_Callback, this, std::placeholders::_1), sub_opt2);
  /*********************************************************************六维力***************************************************************/
  /*****************************************************六维力数据清零**********************************************/
  ...
  Clear_Force_Data_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Clear_Force_Data_Callback, this, std::placeholders::_1), sub_opt2);
  /******************************************************获取六维力数据************************************************/
  /***************************************************传感器受到的外力数据***********************************************/
  ...
  Get_Force_Data_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Get_Force_Data_Callback, this, std::placeholders::_1), sub_opt2);
  /*******************************************************************************end*****************************************************************/
  ...
  Clear_System_Err_Cmd = this->create_subscription<std_msgs::msg::Empty>(
      ...
      std::bind(&RmArm::Arm_Clear_System_Err_Callback, this, std::placeholders::_1), sub_opt2);
  /*******************************************************************************end*****************************************************************/
}
