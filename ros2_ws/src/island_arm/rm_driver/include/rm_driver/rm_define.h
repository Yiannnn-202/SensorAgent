...
static inline int RM_MOVE_SINGLE_BLOCK(int timeout) {
  return timeout;
}  ///<机械臂运动设置，单线程阻塞模式超时时间
...
typedef struct {
  ...
  int port;     ///< 广播的端口号
  int force_coordinate;  ///< 系统外受力数据的坐标系，-1不支持力传感器 0为传感器坐标系
                         ///< 1为当前工作坐标系 2为当前工具坐标系
...
typedef struct {
  ...
  int control_mode[6];  ///<  6个力控方向（Fx Fy Fz Mx My Mz）的模式 0-固定模式 1-浮动模式
                        ///<  2-弹簧模式 3-运动模 4-力跟踪模式 8-力跟踪+姿态自适应模式
  float desired_force
      [6];  ///< 力控轴维持的期望力/力矩，力控轴的力控模式为力跟踪模式时，期望力/力矩设置才会生效
            ///< ，精度0.1N。
...
typedef struct {
  ...
  int control_mode[6];  ///< 6个力控方向的模式 0-固定模式 1-浮动模式 2-弹簧模式 3-运动模式
                        ///< 4-力跟踪模式 5-浮动+运动模式 6-弹簧+运动模式 7-力跟踪+运动模式
                        ///< 8-姿态自适应模式
  float desired_force
      [6];  ///< 力控轴维持的期望力/力矩，力控轴的力控模式为力跟踪模式时，期望力/力矩设置才会生效
            ///< ，精度0.1N。
...
typedef struct {
  ...
  bool follow;                // 表示驱动器的运动跟随效果，true 为高跟随，false
                // 为低跟随。若使用高跟随，透传周期要求不超过 10ms。
...
typedef struct {
  ...
  int error;  ///< 夹爪错误信息，低8位表示夹爪内部的错误信息bit5-7 保留bit4 内部通bit3 驱动器bit2
              ///< 过流 bit1 过温bit0 堵转
  int mode;  ///< 当前工作状态：1 夹爪张开到最大且空闲，2 夹爪闭合到最小且空闲，3 夹爪停止且空闲，4
             ///< 夹爪正在闭合，5 夹爪正在张开，6 夹爪闭合过程中遇到力控停止
...
typedef struct {
  ...
  float
      work_zero_force_data[6];  ///< 当前工作坐标系下系统外受力原始数据，力的单位为N；力矩单位为Nm。
  float
      tool_zero_force_data[6];  ///< 当前工具坐标系下系统外受力原始数据，力的单位为N；力矩单位为Nm。
} rm_force_data_t;
 ...
 */
typedef struct {
  int pos;  ///< 扩展关节角度，单位度，精度 0.001°(若为升降机构高度，则单位：mm，精度：1mm，范围：0
            ///< ~2300)
...
typedef struct {
  ...
  // int err_line;       ///< 若运行失败，该参数返回有问题的工程行数，err_line 为
  // 0，则代表校验数据长度不对
} rm_send_project_t;
...
typedef struct {
  ...
  int size;                      ///< 包络球数量
  char tool_name
      [12];  ///< 控制器中已存在的工具坐标系名称，如果不存在该字段，则为临时设置当前包络参数
...
typedef struct {
  ...
  int dof_state
      [6];  // 各自由度当前状态:0正在松开1正在闭合2位置到位停止3力控到位停止4触觉到位停止5电流保护停止6发生故障
...
typedef struct {
  ...
  char master_name
      [20];  // Modbus
             // 主站名称，最大长度15个字符，不超过15个字符（master_name与IP二选一，若有IP和port优先使用IP和port）
 ...
 *
 */
typedef enum { DOF_TYPE_6 = 6, DOF_TYPE_7 = 7 } rm_dofType_e;
