 ...
 * 机械臂事件回调函数，该回调函数接收rm_event_push_data_t类型的数据作为参数，没有返回值。
 */
 ...
 * @return int 函数执行的状态码。
            ...
            - -1: 数据发送失败，通信过程中出现问题。
            - -2:
 数据接收失败，通信过程中出现问题或者控制器超时没有返回。可能情况：查询的工具坐标系不存在
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
 ...
 * @return int 函数执行的状态码。
            ...
            - -1: 数据发送失败，通信过程中出现问题。
            - -2:
 数据接收失败，通信过程中出现问题或者控制器超时没有返回。例如：查询的工具坐标系不存在
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
 ...
 * @return int 函数执行的状态码。
            ...
            - -5: 单线程模式超时未接收到返回，请确保超时时间设置合理。
 */
RM_INTERFACE_EXPORT int rm_set_joint_step(rm_robot_handle *handle, int joint_num, float step, int v,
                                          int block);
/**
 * @brief 当前工作坐标系下，位置步进
 *
...
RM_INTERFACE_EXPORT int rm_get_controller_state(rm_robot_handle *handle, float *voltage,
                                                float *current, float *temperature, int *err_flag);
/**
 * @brief 设置机械臂电源
 *
 * @param handle 机械臂控制句柄
 ...
 * @param handle 机械臂控制句柄
 * @param version
 （三代控制器）获取到的各关节软件版本号数组，需转换为十六进制，例如获取某关节版本为54536，转换为十六进制为D508，则当前关节的版本号为
 Vd5.0.8
 ...
 * @return int 函数执行的状态码。
            ...
            - -1: 数据发送失败，通信过程中出现问题。
            - -2:
 数据接收失败，通信过程中出现问题或者控制器超时没有返回。可能情况：当前机械臂不是六维力版本。
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
 ...
 * @note 失败的可能原因:
            ...
            - 使用六维力模式拖动示教时，当前已处于奇异区
 */
RM_INTERFACE_EXPORT int rm_start_multi_drag_teach_new(rm_robot_handle *handle,
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_set_force_position_new(rm_robot_handle *handle,
 ...
 * @param block
 设置false时为非阻塞模式；设置true时为阻塞模式，阻塞模式时超时20ms未返回认为接受失败。
 * @return int 函数执行的状态码。
            - 0: 成功。
            - 1: 控制器返回false，传递参数错误或机械臂状态发生错误。
            - -1: 数据发送失败，通信过程中出现问题。
 ...
 *
 * 睿尔曼机械臂在控制器和末端接口板上各提供一个RS485通讯接口，这些接口可通过接口配置为标准的Modbus
 * RTU模式。 在Modbus RTU模式下，用户可通过提供的接口对连接在端口上的外设进行读写操作。
 *
 * @attention
 ...
 *
 * @attention
 如需配置连接ModbusTCP从站，则该接口需要在创建机械臂句柄后调用一次，否则在读写寄存器时无法接收到返回值。
 * @param handle 机械臂控制句柄
 * @param ip 从机IP地址
 * @param port 端口号
 ...
 *
 * @param handle 机械臂控制句柄
 * @param params 多圈数据读取参数结构体，要读的线圈的数量 8< num <= 120，该指令最多一次性支持读 120
 个线圈数据， 即 15 个 byte
 * @param data 返回线圈状态，数据类型：int8
 * @return int 函数执行的状态码。
            - 0: 成功。
 ...
 * @param errline 若运行失败，该参数返回有问题的工程行数，err_line 为
 0，则代表校验数据长度不对，err_line 为 -1，则代表无错误
 * @return int 函数执行的状态码。
            - 0: 成功。
            - 1: 控制器返回false，传递参数错误或机械臂状态发生错误。
            - -1: 数据发送失败，通信过程中出现问题。
 ...
 *            - -4: 三代控制器不支持该接口
 */
RM_INTERFACE_EXPORT int rm_set_controller_rs485_mode(rm_robot_handle *handle, int mode,
                                                     int baudrate);
/**
 ...
 * @return int 函数执行的状态码。
            ...
            - -3: 返回值解析失败，接收到的数据格式不正确或不完整。
 */
RM_INTERFACE_EXPORT int rm_set_movev_canfd_init(rm_robot_handle *handle, int avoid_singularity_flag,
                                                int frame_type, int dt);
/**
 * @brief 读末端生态设备寄存器
 *
 * @param handle 机械臂控制句柄
