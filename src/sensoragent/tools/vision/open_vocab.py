...
def _show_detection_preview(
  ...
  """Block on a GUI window so the first physical scan is operator-visible."""

  try:
    import cv2
    from PIL import Image, ImageDraw
  except ImportError as exc:
    raise RuntimeError("First-detection preview requires Pillow and OpenCV") from exc
  source = Path(image_path)
  image = (
    Image.fromarray(np.load(source)).convert("RGB")
    if source.suffix.casefold() == ".npy"
    else Image.open(source).convert("RGB")
  )
  rendered = image.convert("RGBA")
  colors = ((235, 87, 87), (47, 128, 237), (39, 174, 96), (242, 153, 74))
  mask_layer = Image.new("RGBA", rendered.size, (0, 0, 0, 0))
  mask_draw = ImageDraw.Draw(mask_layer)
  for index, detection in enumerate(detections):
    color = colors[index % len(colors)]
    for polygon in detection.mask_polygons or []:
      mask_draw.polygon([tuple(point) for point in polygon], fill=(*color, 92))
  rendered = Image.alpha_composite(rendered, mask_layer).convert("RGB")
  draw = ImageDraw.Draw(rendered)
  for index, detection in enumerate(detections):
    color = colors[index % len(colors)]
    if detection.bbox_2d is not None:
      draw.rectangle(detection.bbox_2d, outline=color, width=3)
      draw.text(
        (detection.bbox_2d[0] + 4, max(0, detection.bbox_2d[1] - 16)),
        f"{index + 1}. {detection.label} {detection.confidence:.2f}",
        fill=color,
      )
  window_name = "SensorAgent: first YOLO-Seg detection (close, Esc, or q to continue)"
  try:
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    cv2.imshow(window_name, cv2.cvtColor(np.asarray(rendered), cv2.COLOR_RGB2BGR))
  except cv2.error as exc:
    raise RuntimeError("First-detection preview could not open a GUI window") from exc
  try:
    while True:
      try:
        visible = cv2.getWindowProperty(window_name, cv2.WND_PROP_VISIBLE)
      except cv2.error:
        # Qt destroys its receiver as soon as the user closes the window.
        # That is the expected operator acknowledgement, not a vision failure.
        break
      if visible < 1 or cv2.waitKey(100) & 0xFF in {27, ord("q")}:
        break
  finally:
    try:
...
def _resolve_spatial(
  ...
  if relation in _SPATIAL_IMAGE_MIDDLE_RELATIONS:
    ...
    if constraint.ordinal > len(keyed):
      return None, candidates, "ambiguous"
    ordinal = 2 if constraint.ordinal == 1 and len(keyed) >= 2 else constraint.ordinal
    winner = keyed[ordinal - 1][1]
...
class VisionOpenVocabularyDetectTool:
  ...
  def _cloud_base_xy_estimator(
    ...
    """Build an exact candidate XY estimator from a hardware xyzuv point cloud."""
    ...
    if cloud.ndim != 2 or cloud.shape[0] == 0 or cloud.shape[1] < 5:
      raise ValueError("cloud_path must contain Nx5 xyzuv points")
    cloud = cloud[np.isfinite(cloud[:, :5]).all(axis=1)]
    if cloud.size == 0:
      return None
    ...
    def estimate(detection: VisionDetection) -> tuple[float, float] | None:
      ...
      if center is None or len(center) < 2:
        ...
        center = [(bbox[0] + bbox[2]) / 2.0, (bbox[1] + bbox[3]) / 2.0]
      distances = (cloud[:, 3] - center[0]) ** 2 + (cloud[:, 4] - center[1]) ** 2
      position_base = _transform_to_base(
        cloud[int(np.argmin(distances)), :3].tolist(),
        t_base_camera,
      )
      if position_base is None:
        return None
      return position_base[0], position_base[1]
  ...
  def _select_candidates(
    ...
    *,
    ...
    """Collect candidates, admit only configured source regions, then rank them."""
    ...
    base_xy_of = self._cloud_base_xy_estimator(cloud_path, t_base_camera)
    if base_xy_of is None:
    ...
    if self._allowed_xy_polygon is not None:
      # A source region is a safety boundary: missing geometry must reject,
      # rather than permit a candidate that cannot be located in base_link.
      if base_xy_of is None:
  ...
  def run(self, call: ToolCall) -> ToolResult:
    ...
    try:
      ...
      if self._first_detection_preview_gui and not self._first_detection_preview_shown:
        if image_path is None:
          raise ValueError("image_path is required for the first-detection preview")
        if not isinstance(self._detector, UltralyticsOpenVocabularyBackend):
          raise RuntimeError("First-detection preview requires a fixed-class Ultralytics model")
        preview_detections = self._detector.detect_all(
          query=query,
          image_path=image_path,
          depth_path=depth_path,
          options=options,
        ...
        if self._allowed_xy_polygon is not None:
          preview_base_xy = self._cloud_base_xy_estimator(
            call.input.get("cloud_path"),
            call.input.get("T_base_camera", self._t_base_camera),
          )
          if preview_base_xy is not None:
            preview_detections = [
              candidate
              for candidate in preview_detections
              if _within_allowed_xy_polygon(
                preview_base_xy(candidate),
                self._allowed_xy_polygon,
                self._allowed_xy_margin_m,
              )
            ]
            ...
            preview_detections = []
        _show_detection_preview(image_path, preview_detections)
        self._first_detection_preview_shown = True
      ...
      elif spatial is not None or self._allowed_xy_polygon is not None:
        detection, alternatives, spatial_error = self._select_candidates(
          query=query,
          image_path=image_path,
          depth_path=depth_path,
          ...
          if spatial is None:
            return ToolResult(
              ...
              output={
                ...
                "confidence": 0.0,
                "source": self._backend,
                "candidates": [candidate.to_output() for candidate in alternatives],
              },
              error="OBJECT_NOT_FOUND: no matching candidate inside configured source region",
            )
