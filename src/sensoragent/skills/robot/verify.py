...
def _read_gripper_opening(context: SkillContext, call: SkillCall) -> tuple[bool, float, dict, str | None]:
  ...
  if not isinstance(opening, (int, float)):
    return False, 0.0, state, "gripper.get_state did not report a numeric opening"
  return True, float(opening), state, None
...
class RobotVerifyPlaceSkill:
  ...
  def run(self, call: SkillCall, context: SkillContext) -> SkillResult:
    ...
    ok, opening, _, error = _read_gripper_opening(context, call)
    if not ok:
      return SkillResult(skill=self.spec.name, success=False, error=error)
