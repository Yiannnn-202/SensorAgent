...
历史节点安排见[8 月 10 日视觉交付计划](../planning/vision-aug10-delivery.md)，SAM3 是否适合替换
...
这次第一轮训练直接微调 Grounding DINO 本体。SAM2 继续根据检测框生成掩码，
YOLO11n-seg 不纳入当前开发链路；SAM3 只作为基础方案完成后的独立创新对照，不影响
8 月 10 日前交付。

目前还不能说“比赛视觉模型已经训练完成”。现阶段跑通的是预训练模型推理和训练工程
链路，正式比赛模型还缺最终类别确认、足量仿真/真实图片、人工复核标注和冻结测试集。
仿真数据由仿真侧提供 10～20 张初始图片。我收到图片、类别/场景信息和可用检测框后
立即启动第一轮快速微调；截至 2026-07-31 仓库和机器人目录中还没有发现这批正式交付，
所以当前不能把比赛数据训练写成已完成。
...
| [#14](https://github.com/Yiannnn-202/SensorAgent/pull/14) | YOLO11n-seg 历史实验入口、数据预检和测试 | 已合入 `main`，当前不启用 |

这次 Grounding DINO 训练纠正工作在：

| 分支/PR | 内容 | 状态 |
| --- | --- | --- |
| [#16](https://github.com/Yiannnn-202/SensorAgent/pull/16) | Grounding DINO 直接微调、JSONL 训练数据预检、配置模板、结果追踪和文档纠正 | 已合入 `main` |

PR #14 已经合入，里面的 `scripts/vision_train.py` 是历史 YOLO11n-seg 实验入口，
不属于当前 8 月 10 日交付链路。当前唯一主训练入口是
`scripts/vision_train_grounding_dino.py`。PR #16 合入后，对应本地已合入实验分支已经
安全删除，代码仍完整保留在 `main`。

## 3. 我现在采用的模型分工

我现在先直接微调开放词汇主模型；基础交付完成后，再单独做 SAM3 对照：
...
flowchart LR
  ...
  box --> sam["SAM 2 细化掩码"]
  dino --> test["固定测试集评测"]
  test -->|"基础交付完成后"| sam3["SAM3 同测试集创新对照"]
```
...
- **YOLO11n-seg**：不进入当前开发链路，也不作为 8 月 10 日交付物；仓库中的旧入口
  只保留历史兼容，不在这一阶段维护或训练。
- **SAM3**：已完成官方资料核验，支持文本提示直接输出框和掩码，但当前没有本机权重、
  适配代码或同测试集数据。它作为后续独立创新对照，不直接替换已跑通方案。
...
`models/asr/vad/silero_vad.onnx`；一个是 recovery 测试需要未启动的
`127.0.0.1:8765` robot bridge；另一个是最新主线 DecisionTree `last_failure`
记录节点与测试预期不同。视觉专项测试保持通过。
...
第一批 10～20 张仿真图片只用于快速微调和过拟合式链路验证。数据到位且格式可用后，
我当天开始预标注和训练，首轮结果预计当天或第二天给出。8 月 10 日前交付可重复运行的
仿真模型流程、可启动训练的数据结构和创新点初稿；真实工业稳定版仍要根据漏检、误检和
真实工位困难样本继续迭代。
