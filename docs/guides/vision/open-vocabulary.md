...
flowchart LR
  ...
  box --> sam["SAM 2 细化掩码"]
  dino --> assess["固定测试集评测"]
  assess -->|"基础交付完成后"| sam3["SAM3 同测试集创新对照"]
```
...
基础交付完成后，再把 SAM3 作为独立对照。SAM3 必须与当前主模型在完全相同的 test
split 上比较。至少记录是否微调、是否使用 SAM2、模型尺寸、输入分辨率、精度、延迟、
显存和文件大小，不能只报“更快”或“更准”。预训练 Grounding DINO 和 SAM2 的输出都
只能作为标注草稿，
