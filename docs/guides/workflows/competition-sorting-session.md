...
为参考。当前受控混排为每个同类实例提供不同的 X/Y 位置，因此可使用：
...
如果两个候选在所选关系上的坐标相同或近似相同，系统仍会拒绝猜测并要求
换一种空间描述。
...
- 同类物体尺寸相同却要求按最大/最小选择；
- 目标格不存在或已经被本会话中的物体占用；
- 批量指令未给出起始目标格，或剩余空格不足以容纳全部候选。

系统会返回 `needs_clarification` 或 `unsupported` 以及具体原因，而不是默认
选择中间物体。

“所有/全部”批量任务已支持。例如“把所有滚轮放入一号格”会按配置的实例顺序
生成子任务，并从指定起始格开始依次使用空格。已放置对象会从后续候选中排除；
任一子任务失败时，会记录失败对象和剩余队列并停止该批次。
