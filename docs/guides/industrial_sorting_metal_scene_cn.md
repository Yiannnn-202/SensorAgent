...
| 类别 | X 坐标 | Y 坐标 | 姿态 |
| --- | --- | --- | --- |
| 短螺栓 | `0.1325 / 0.2325 / 0.3325` | `-0.04` | 头部朝下、细杆朝上 |
| 六角螺母 | `0.1325 / 0.2325 / 0.3325` | `-0.13` | 竖直，偏航角分别为 `-15 / 0 / 15` 度 |
| 滚轮 | `0.1325 / 0.2325 / 0.3325` | `-0.22` | 横放，长轴沿 world X |

100 mm 的同排间距和 90 mm 的排间距在保持原工作区的同时，为 Robotiq 2F-85 的俯视抓取留出间隙。所有零件相对上一版沿 world X 正方向平移 12.5 mm，即远离机械臂一个短螺栓杆半径。零件使用低高光浅灰材质，减少视觉模型对类别颜色的依赖。3x3 料箱位于另一侧且初始为空；料箱为 `351 x 351 mm`，每个格位约有 `105 x 105 mm` 的净宽和 `55 mm` 外壁。
