...
RM65-B 的基座安装面为 `z=0.25 m`，比 `z=0.30 m` 的工作台顶面低
`0.05 m`。工作台前沿位于机械臂基座前方约 `0.15 m`，为底部关节旋转保留
空间。该安装高度写入组合 URDF 的
...
空场景仍保留机械臂底座，以匹配组合 URDF 中 `0.25 m` 的安装高度，但不加载
工作台、料箱、零件和相机。
