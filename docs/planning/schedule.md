...
> **2026-08-17 状态更新：**下方甘特图和 7 月 31 日判断保留为历史排期基线，
> 不是当前能力清单。当前代码已增加安全门控的 RM65 + OmniPicker 真机适配、
> 硬件 RGB-D 单物体抓取和多实例批量队列；仍缺可重复 Gazebo/真机批量验收、
> 实机放置/分拣/恢复、标定误差和量化成功率证据。当前能力以根目录
> `README.md`，当前待办以 `TODO.md` 为准。
...
因此 7 月 31 日联调节点为**部分通过**，8 月 10 日阶段门存在进度风险。该判断
不包含 8 月 17 日以后合入的硬件和批量队列代码。
