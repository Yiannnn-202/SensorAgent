...
我这边已经先完成了一轮 `red_block_v0` 单类别 Grounding DINO 全参数微调，并把权重重新加载到 `vision.grounded_sam2` 链路中验证。这个结果把训练脚本、checkpoint 加载和框到掩码的基本流程跑通了，但它使用的是本地 `red block` 小数据集，不是最终的多类别仿真比赛数据。

目前可以确认：训练流程和视觉 Tool 链路已经有一版权重可交接；还不能确认：多类别比赛模型、真实工位泛化、RGB-D 三维定位和机械臂抓取成功率。详细参数、哈希和评测结果见 [red_block_v0 训练交接记录](../guides/vision/grounding-dino-red-block-v0-cn.md)。
...
本项输入负责人是钱奕安。按 7 月 31 日讨论，他应在次日（2026-08-01）向许浩翔提供
第一批少量仿真数据，用于 Grounding DINO + SAM 2 的初步训练验证。截至 7 月 31 日本次
复核时，本地目录和 GitHub 远端均未发现正式交付，因此状态仍是“待提供”，不是“已接收”。
...
收到后由许浩翔检查文件可读性、类别命名、`scene_id`、检测框/掩码可用性和数据来源，
检查通过后再生成训练 JSONL 并启动初步验证。没有这次实际交付前，不能把比赛仿真数据
微调列为已完成。
